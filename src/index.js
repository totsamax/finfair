            ...
            $.ajax(settings).always(function(response) {
              if (response == 'Error: code invalid') {
                ...
                if (
                  ...
                  data.inputAge == '15-17' ||
                  (data.inputJob == 'Не работаю' &&
                    data.inputSalary == '0-50 тыс.руб.')
                ) {
                  $(regForm).html(
                    '<h3>Спасибо! Ваша заявка будет рассмотрена организатором, в случае подтверждения вам придет уведомление на указанный вами адрес электронной почты в течение 3 рабочих дней</h3>'
                  );
                } else {
                  $(regForm).html(
